...
static int concat_append_file(AVFormatContext *avf,const char *filename)
{
    ConcatContext *cat = avf->priv_data;
    int i;
    ...
    char* fname = av_strdup(filename);
    if ((ret = add_file(avf, fname, &file, &nb_files_alloc)) < 0){
        return -1;
    }
