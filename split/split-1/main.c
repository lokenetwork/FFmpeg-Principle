...
int main()
{
    ...
    AVFilterContext *mainsrc_ctx,*split_ctx,*scale_ctx,*overlay_ctx,*resultsink_ctx = NULL;
            ...
            if( AVERROR(EAGAIN) == ret ){
                ...
                if( NULL == filter_graph ){
                    ...
                    ret = avfilter_graph_create_filter(&resultsink_ctx,avfilter_get_by_name("buffersink"),
                                                       "Parsed_buffer_1_888", NULL, NULL,filter_graph);
                    if (ret < 0) {
                    ...
                    //创建 split 滤镜 ctx
                    ret = avfilter_graph_create_filter(&split_ctx,
                                                       avfilter_get_by_name("split"),
                                                       "Parsed_split_2_777", "outputs=2", NULL,filter_graph);
                    if (ret < 0) {
                        printf("split ctx fail \n");
                        return ret;
                    }

                    //连接 buffer滤镜 跟 split 滤镜的第一个输出流
                    if ((ret = avfilter_link(mainsrc_ctx, 0, split_ctx, 0)) < 0){
                    ...
                    av_bprintf(&args,"%d:%d", frame->width/4, frame->height/4);
                    ret = avfilter_graph_create_filter(&scale_ctx,
                                                       avfilter_get_by_name("scale"),
                                                       "Parsed_scale_3_444", args.str, NULL,filter_graph);

                    //连接 split 滤镜的第一个输出流到 scale 滤镜的第一个输入流。
                    if ((ret = avfilter_link(split_ctx, 0, scale_ctx, 0)) < 0){
                        printf("link ctx fail\n");
                        return ret;
                    }
                    ...
                    //创建 overlay 滤镜
                    av_bprint_clear(&args);
                    av_bprintf(&args,"%d:%d", frame->width/4*3, frame->height/4*3);
                    ret = avfilter_graph_create_filter(&overlay_ctx,
                                                       avfilter_get_by_name("overlay"),
                                                       "Parsed_scale_4_444", args.str, NULL,filter_graph);


                    //连接 scale 滤镜的第一个输出流 跟 overlay_ctx 滤镜的第二个输入流
                    if ((ret = avfilter_link(scale_ctx, 0, overlay_ctx, 1)) < 0){
                        printf("link ctx fail1\n");
                        return ret;
                    }

                    //连接 split 滤镜的第二个输出流到 overlay_ctx 滤镜的第一个输入流。
                    if ((ret = avfilter_link(split_ctx, 1, overlay_ctx, 0)) < 0){
                        printf("link ctx fail\n");
                        return ret;
                    }

                    //连接 overlay_ctx 滤镜的第一个输出流 到 resultsink_ctx 滤镜
                    if ((ret = avfilter_link(overlay_ctx, 0, resultsink_ctx, 0)) < 0){
                        printf("link ctx fail\n");
                        return ret;
                    }
